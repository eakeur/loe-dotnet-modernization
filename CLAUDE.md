...
Scoped per Blazor circuit (one per connected browser tab, so different assessors/tabs never share mutable state). Owns the currently-loaded `SolutionModel`/`DependencyGraph`/`UsageResults`, exposes a `Changed` event every page subscribes to for re-render, and — once a solution is loaded — starts a debounced `FileSystemWatcher` on the solution's directory tree that automatically re-parses on any `.sln`/`.slnf`/`.csproj`/`Directory.Build.*`/`Directory.Packages.props`/`NuGet.Config` change (this is the "live" re-scan feature; it's cheap thanks to the evaluation cache described above).
